...
int logicalShift(int x, int n) {
  /*
   * Use a mask to make left n bit become 0. 
   */
  int mask = ~(((1 << 31) >> n) << 1);
  return (x >> n) & mask;
}
...
int bang(int x) {
  /* 
   * For x = 0, x = -x. So we can check if MSB of x and -x are all 0;
   */
  int invertedX = ~x + 1;
  int zeroOrPositive = (x >> 31) + 1;
  int zeroOrNegative = (invertedX >> 31) + 1;
  return zeroOrPositive & zeroOrNegative;
}
/* 
...
int fitsBits(int x, int n) {
  /*
   * Get truncated x of n-bit and check if equals x.
   */
  int shift = 33 + ~n;
  int truncatedX = (x << shift) >> shift;
  return !(x ^ truncatedX);
}
/* 
...
int divpwr2(int x, int n) {
    /*
     * Compute (x + 2^k - 1) / 2^k then it's correct for x < 0.
     */
    int isNegative = x >> 31;
    int plus = ((isNegative & 1) << n) + isNegative;
    return (x + plus) >> n;
}
/* 
 * negate - return -x 
...
int isPositive(int x) {
  /*
   * Check if it's negative or zero.
   */
  int zeroOrPositive = (x >> 31) + 1;
  int notZero = !!x;
  return zeroOrPositive & notZero;
}
/* 
...
int isLessOrEqual(int x, int y) {
  /*
   * x <= y means y - x is zero or positive.
   * Also if x and y have different sign, we need to avoid overflow.
   */
  int xIsNegative = (x >> 31) & 1;
  int yIsNegative = (y >> 31) & 1;
  int differentSign = xIsNegative ^ yIsNegative;

  int negX = ~x + 1;
  int sum = negX + y;
  int lessOrEqual = (sum >> 31) + 1;
  return (differentSign & xIsNegative) | (!differentSign & lessOrEqual);
}
/*
...
int ilog2(int x) {
  /*
   * This is to find leftmost 1. Use binary search to minimize operations. 
   */
  int result = (!!(x >> 16)) << 4;
...
unsigned float_neg(unsigned uf) {
  ...
  if (((expMask & uf) == expMask) && (fracMask & uf)) {
    return uf;
  }
  return uf ^ mask;
}
